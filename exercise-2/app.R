...
diamonds.price.range <- range(diamonds$price)
diamonds.carat.range <- range(diamonds$carat)
...
my.ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      sliderInput("price.key", "Price (in dollars)", diamonds.price.range[1], diamonds.price.range[2]),
      ...
      sliderInput("carats.key", "Carats", diamonds.carat.range[1], diamonds.carat.range[2])
